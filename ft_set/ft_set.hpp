...
/*   Updated: 2021/06/30 16:15:14 by fignigno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include "./ft_set_bidirectional_iterator.hpp"
...
namespace ft {
	...
	public:
		...
		typedef	SetBidirectIterator<value_type, node>			iterator;
		typedef	SetBidirectIterator<const value_type, node>		const_iterator;
		...
		ft::pair<iterator, bool>	insert(const value_type &val) {
			...
			ft::pair<node *, node *>	is_in_set = find_node(val);
			if (!isNIL(is_in_set.first))
				return (ft::make_pair<iterator, bool>(is_in_set.first, false));
			x = create_node(val);
			if (isNIL(is_in_set.second))
				parent = 0;
			else
				parent = is_in_set.second;
			x->parent = parent;
			if (parent)
				init_leaf(parent, x);
			else
